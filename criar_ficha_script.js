// criar_ficha_script_final_com_tecnicas.js
...
// Variáveis para controlar o modo de agrupamento de técnicas
let modoAgrupamento = {
    ativo: false,
    tecnica: null,
...
function atualizarListaExercicios() {
    ...
    if (exerciciosAdicionados.length === 0) { listaExerciciosDiv.innerHTML = `<div class="empty-state"><div class="empty-icon">📋</div><div><strong>Nenhum exercício adicionado ainda</strong> Selecione exercícios para começar a montar a ficha.</div></div>`; return; }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (event.target.closest('#btn-editar-ficha')) { if (!event.target.closest('#btn-editar-ficha').disabled) { if (fichaSelecionada) { iniciarEdicaoFicha(fichaSelecionada.id); } else { alert('Por favor, selecione uma ficha para editar.'); } } }
        else if (event.target.closest('#btn-nova-ficha')) { iniciarNovaFicha(); }
        else if (event.target.closest('#btn-salvar-ficha')) { salvarFichaOnline(); }
        else if (event.target.closest('#btn-gerar-pdf')) { gerarPDF(); }
        else if (event.target.closest('.add-exercise-btn[onclick="adicionarExercicio()"]')) { adicionarExercicio(); }
